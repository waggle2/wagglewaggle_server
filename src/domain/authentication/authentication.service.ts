...
export class AuthenticationService {
  ...
  async getCookieWithAccessToken(user: User) {
    ...
    return `accessToken=${token}; HttpOnly; Max-Age=${process.env.JWT_EXPIRATION_TIME}; Path=/`; // JWT 토큰을 쿠키 형태로 반환
  }
  ...
  async getCookieWithRefreshToken(user: User) {
    ...
    return `refreshToken=${token}; HttpOnly; Max-Age=${process.env.JWT_REFRESH_EXPIRATION_TIME}; Path=/`;
  }
